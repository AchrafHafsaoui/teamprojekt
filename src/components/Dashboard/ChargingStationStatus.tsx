...
const ChargingStationStatus: React.FC = () => {
    ...
    const filteredStations = filterStatus === 'all' ? stations : stations.filter(station => station.availability === filterStatus);
    ...
    const renderGraduatedBar = (chargingPower: number) => {
        ...
        const [animatedSegments, setAnimatedSegments] = useState<number[]>(Array(segmentCount).fill(0));
                ...
                {Array.from({ length: segmentCount }).map((_, index) => (
                    <div
                        ...
                        style={{
                            ...
                            backgroundColor: animatedSegments[index] ? '#078ECD' : 'bg-gray-300',
                        }}
                    ></div>
                ))}
            </div>
            ...
            <div className="overflow-y-auto overflow-x-hidden h-80 custom-scrollbar">
                ...
                <div className="space-y-3">
                    {filteredStations.map((station) => (
                        <div key={station.stationId} className="grid grid-cols-3 gap-4 items-center text-gray-800 pb-3 shadow-sm font-semibold">
                            ...
                            <div className="w-full">
                                ...
                                <div className="text-center font-semibold mt-1 text-sm">{station.chargingPower} kW</div>
                            </div>
                        </div>
                    ))}
                </div>
