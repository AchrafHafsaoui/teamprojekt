...
const ChargingSchedule0: React.FC = () => {
  ...
  // Calculate totals
  const totalEnergyUsed = vehicles.reduce(
    (sum, vehicle) => sum + vehicle.energyUsed,
    0,
  );
  const totalEnergyRemaining = vehicles.reduce(
    (sum, vehicle) => sum + vehicle.energyRemaining,
    0,
  );
  const totalChargingPower = vehicles.reduce(
    (sum, vehicle) => sum + vehicle.chargingPower,
    0,
  );
  ...
  // Flexibility ratios
  const upwardFlexibility =
    ((totalCapacity - totalEnergyUsed) / totalCapacity) * 100;
    ...
    <div className="bg-[#FFFFFF] bg-opacity-80 h-full flex flex-col border border-[#D3D3D3] shadow-md rounded-3xl p-6">
      <div className="mb-4">
        ...
        <div className="flex justify-between items-center">
          <p className="text-lg">
            Total Energy Used:{" "}
            <span className="font-bold">{totalEnergyUsed} kWh</span>
          </p>
          <p className="text-lg">
            Total Energy Remaining:{" "}
            <span className="font-bold">{totalEnergyRemaining} kWh</span>
          </p>
          <p className="text-lg">
            Total Charging Power:{" "}
            <span className="font-bold">{totalChargingPower} kW</span>
          </p>
        </div>
      </div>
...
export default ChargingSchedule0;
