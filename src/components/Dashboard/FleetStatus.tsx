...
const FleetStatus: React.FC<FleetStatusProps> = ({ fullPage = true }) => {
  ...
  const currentBuses = vehicles.slice(
  ...
  return (
    ...
    >
      ...
      <div
        className={`overflow-x-hidden ${fullPage ? "h-full" : "h-80"} custom-scrollbar`}
      >
        {/* Header Row */}
        <div
          className={`grid gap-4 font-bold text-lg mb-2 px-2 text-gray-600 text-center ${fullPage ? "grid-cols-7" : "grid-cols-5"}`}
        >
        ...
        <div className="space-y-3">
          {currentBuses.map((vehicle, index) => (
            <div
              key={vehicle.busId}
              className={`grid gap-4 items-center text-gray-800 pb-3 shadow-sm font-semibold ${
            ...
            >
              ...
              >
                ...
                {vehicle.status == "Maintenance" && (
                  <div className="absolute bottom-1 text-center font-bold">
                    <svg
                      fill="#000000"
                      width="25"
                      height="25"
