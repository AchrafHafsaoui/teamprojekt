...
const FleetStatus: React.FC = () => {
    const vehicles: VehicleData[] = [
        { busId: '23001', availability: 'Charging', battery: 50, status: 'In Depot', charging: true, CAP: '30MWh', ENE: '50MWh' },
        { busId: '19101', availability: 'Unvailable', battery: 0, status: 'In Maintenance', charging: false, CAP: '20MWh', ENE: '30MWh' },
        { busId: '15401', availability: 'Available', battery: 30, status: 'On Route', charging: true, CAP: '100MWh', ENE: '0MWh' },
        { busId: '18757', availability: 'Available', battery: 95, status: 'On Route', charging: false,CAP: '15MWh', ENE: '70MWh' },
        { busId: '19289', availability: 'Charging', battery: 65, status: 'In Depot', charging: true, CAP: '30MWh', ENE: '10MWh' },
        { busId: '21001', availability: 'Unvailable', battery: 50, status: 'In Maintenance', charging: false, CAP: '95MWh', ENE: '75MWh' },
        { busId: '22345', availability: 'Available', battery: 80, status: 'On Route', charging: false, CAP: '15MWh', ENE: '50MWh' },
        { busId: '23456', availability: 'Charging', battery: 45, status: 'In Depot', charging: true, CAP: '0MWh', ENE: '40MWh' },
    ];

    const [filterStatus, setFilterStatus] = useState<'all' | 'In Depot' | 'In Maintenance' | 'On Route'>('all');
    ...
    const filteredVehicles = filterStatus === 'all' ? vehicles : vehicles.filter(vehicle => vehicle.status === filterStatus);

    const renderStatusIcon = (status: 'In Depot' | 'In Maintenance' | 'On Route') => {
        switch (status) {
            case 'In Depot':
                return (
                ...
                <div className="flex space-x-3">
                    {['all', 'In Depot', 'In Maintenance', 'On Route'].map((status) => (
                        <button
                            ...
                            style={{ backgroundColor: filterStatus === status ? 'rgba(7, 142, 205, 0.8)' : undefined }}
                        >
            ...
            <div className="overflow-x-hidden h-80 custom-scrollbar">
                ...
                <div className="space-y-3">
                    {filteredVehicles.map((vehicle, index) => (
                        <div key={vehicle.busId} className="grid grid-cols-5 gap-4 items-center text-gray-800 pb-3 shadow-sm font-semibold">
